...
try:
    ...
    server = MinecraftServer(config_dic['server-path'], config_dic['ram']['min-ram'], config_dic['ram']['max-ram'])
    ...
    def isallowed():
        ...
        async def predicate(ctx):
            ...
            if ctx.author.display_name.lower() in config_dic["allowed-names"] or ctx.author.name.lower() in ' '.join(config_dic["allowed-names"]).lower():
                return True
    ...
    async def mainMinecraft(ctx):
        ...
        if(comanda == "start"):
            ...
            return await chan.send(f"```This is the Minecraft Server IP : {server.ip()}:25565```")
